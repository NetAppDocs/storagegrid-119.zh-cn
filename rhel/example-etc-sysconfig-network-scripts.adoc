...
summary: 您可以使用示例文件将四个 Linux 物理接口聚合为单个 LACP 绑定，然后建立三个与该绑定对应的 VLAN 接口，以用作StorageGRID网格、管理和客户端网络接口。 
---
...
[role="lead"]
您可以使用示例文件将四个 Linux 物理接口聚合为单个 LACP 绑定，然后建立三个与该绑定对应的 VLAN 接口，以用作StorageGRID网格、管理和客户端网络接口。
