...
summary: 如果在 Linux 主机上部署了任何StorageGRID节点，请在开始升级之前在每个主机上安装额外的 RPM 或 DEB 包。 
---
= Linux：在所有主机上下载并安装 RPM 或 DEB 包
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果在 Linux 主机上部署了任何StorageGRID节点，请在开始升级之前在每个主机上下载并安装额外的 RPM 或 DEB 包。



== 下载升级、Linux 和修补程序文件

当您从网格管理器执行StorageGRID升级时，系统会提示您下载升级存档和任何所需的修补程序作为第一步。但是，如果您需要下载文件来升级 Linux 主机，则可以提前下载所有必需的文件以节省时间。
...
+
StorageGRID软件版本的格式如下：11._x.y_。  StorageGRID修补程序具有以下格式：11._x.y.z_。

. 使用您的NetApp帐户的用户名和密码Sign in。
. 如果出现“注意/必读”通知，请记下修补程序编号，然后选中复选框。
. 阅读最终用户许可协议 (EULA)，选中复选框，然后选择*接受并继续*。
+
出现您所选版本的下载页面。该页面包含三列。

. 从第二列（*升级StorageGRID*）下载两个文件：
+
** 最新版本的升级档案（这是标有 *VMware、SG1000 或 SG100 主管理节点* 的部分中的文件）。虽然直到执行升级时才需要此文件，但现在下载它将节省时间。
** RPM 或 DEB 档案 `.tgz`或者 `.zip`格式。选择 `.zip`如果您在服务笔记本电脑上运行 Windows，则该文件。
+
*** 红帽企业 Linux +
...
. 如果您因为需要修补程序而需要同意“警告/必读”通知，请下载修补程序：
+
.. 返回 https://mysupport.netapp.com/site/products/all/details/storagegrid/downloads-tab["NetApp下载： StorageGRID"^]。
.. 从下拉菜单中选择修补程序编号。
.. 再次同意警告通知和 EULA。
.. 下载并保存修补程序及其自述文件。
+
当您开始升级时，系统将提示您在StorageGRID升级页面上上传修补程序文件。
...
在升级StorageGRID软件之前执行以下步骤。

.步骤
. 从安装文件中提取 RPM 或 DEB 包。
. 在所有 Linux 主机上安装 RPM 或 DEB 包。
+
请参阅安装说明中安装StorageGRID主机服务的步骤：

+
** link:../rhel/installing-storagegrid-webscale-host-service.html["Red Hat Enterprise Linux：安装StorageGRID主机服务"]
** link:../ubuntu/installing-storagegrid-webscale-host-services.html["Ubuntu 或 Debian：安装StorageGRID主机服务"]
...
为了释放 Linux 主机上的空间，您可以删除不再需要的以前版本的StorageGRID安装档案。
...
.Red Hat
--
. 捕获已安装的StorageGRID包列表： `dnf list | grep -i storagegrid` 。
+
示例：
...
CAUTION: 请勿删除您当前正在运行的StorageGRID版本或您计划升级到的StorageGRID版本的安装档案。

+
您可以放心地忽略出现的警告。它们指的是安装较新的StorageGRID包时已被替换的文件。
...
.Ubuntu 和 Debian
--
. 捕获已安装的StorageGRID包列表： `dpkg -l | grep storagegrid`
+
示例：
...
[root@docker-example ~]#
----
. 删除以前StorageGRID版本的容器映像： `docker rmi _image id_`
+

CAUTION: 请勿删除您当前正在运行的StorageGRID版本或您计划升级到的StorageGRID版本的容器映像。
...
[root@podman-example ~]#
----
. 删除以前StorageGRID版本的容器映像： `podman rmi _image id_`
+
