...
summary: 您可以使用 S3 SelectObjectContent 请求根据简单的 SQL 语句过滤 S3 对象的内容。 
---
= 选择对象内容
:allow-uri-read: 
:icons: font
...
[role="lead"]
您可以使用 S3 SelectObjectContent 请求根据简单的 SQL 语句过滤 S3 对象的内容。

有关详细信息，请参阅 https://docs.aws.amazon.com/AmazonS3/latest/API/API_SelectObjectContent.html["Amazon Simple Storage Service API 参考：SelectObjectContent"^]。

.开始之前
* 租户账户具有S3 Select权限。
* 你有 `s3:GetObject`您要查询的对象的权限。
* 您要查询的对象必须采用以下格式之一：
+
...
此查询获取州名称、2010 年人口、估计 2015 年人口以及美国人口普查数据的变化百分比。文件中非状态的记录将被忽略。
...
----
要查询的文件的前几行， `SUB-EST2020_ALL.csv` ，看起来像这样：
