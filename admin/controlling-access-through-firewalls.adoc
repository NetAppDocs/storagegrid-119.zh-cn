...
如果要配置StorageGRID内部防火墙，请参阅link:../admin/configure-firewall-controls.html["配置内部防火墙"]。
...
TIP: 受限的网格管理器或租户管理器端口上不提供单点登录 (SSO)。如果您希望用户通过单点登录进行身份验证，则必须使用默认 HTTPS 端口 (443)。

.相关信息
* link:signing-in-to-grid-manager.html["Sign in网格管理器"]
* link:creating-tenant-account.html["创建租户帐户"]
* link:../network/external-communications.html["外部沟通"]
